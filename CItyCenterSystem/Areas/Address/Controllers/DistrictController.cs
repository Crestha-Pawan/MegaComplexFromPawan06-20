        ...
        public async Task<IActionResult> Update(DistrictDto dto)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    return RedirectToAction("Index", "District", new { message = "District has been update successfully." });
                }
                else
                {
                    ViewBag.Message = "Error: Invalid data !";
        ...
        public async Task<IActionResult> DeleteConfirmed(District district)
        {
            try
            {
                if (district != null)
                {
                    ...
                    return RedirectToAction("Index", "District", new { messege = "District has been delete successfully." });
                }
            }
            catch (Exception ex)
            {
